@tool
class_name OutlinePostProcess extends CompositorEffect
...
@export_group("Outlines", "outline_")
@export_range(0.0, 1.0, 0.001) var outline_thickness: float = 0.25
@export var outline_color: Color
@export_range(0.0, 0.05, 0.0001) var outline_step_edge: float = 0.04
...
func _render_callback(_effect_callback_type: int, render_data: RenderData) -> void:
	...
	push_constants.append(outline_color.r)
	push_constants.append(outline_color.g)
	push_constants.append(outline_color.b)
	push_constants.append(1.0)
	push_constants.append(inv_proj_mat[2].w)
	push_constants.append(inv_proj_mat[3].w)
	push_constants.append(outline_thickness)
	push_constants.append(outline_step_edge)
	push_constants.append(0.0)
	push_constants.append(0.0)

	for view: int in scene_buffers.get_view_count():
...
func initialize_compute_shader() -> void:
	...
	var glsl_file: RDShaderFile = load("res://Assets/Shaders/holder/OutlinePostProcess.glsl")
